        ...
        <box className={`notification-card ${urgency(n)}`} vertical>
            <box className="notif-header">
                {n.appIcon || n.desktopEntry
                    ...
                    : <box />}
                <label className="notif-app-name" label={n.appName || "Notification"} xalign={0} hexpand />
            ...
            <box className="notif-body" vertical>
                <label className="notif-summary" label={n.summary} xalign={0} wrap maxWidthChars={34} />
                {n.body
                    ? <label className="notif-text" label={n.body} xalign={0} wrap useMarkup maxWidthChars={34} />
                    : <box />}
            </box>
    ...
    return (
        <box className="qs-section notifications" vertical>
            <box className="qs-section-header">
                <label className="qs-section-title" label="Notifications" xalign={0} hexpand />
                <button
                    className="qs-dnd-toggle"
                    tooltipText="Do not disturb"
                    onClicked={() => (notifd.dontDisturb = !notifd.dontDisturb)}>
                    <label label={bind(notifd, "dontDisturb").as((d) => (d ? "\u{f009b}" : "\u{f009a}"))} />
                </button>
                <button
            ...
            <scrollable className="notif-history" heightRequest={220} vscroll={Gtk.PolicyType.AUTOMATIC} hscroll={Gtk.PolicyType.NEVER}>
                <box vertical>
