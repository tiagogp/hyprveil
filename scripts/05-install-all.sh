...
echo
echo "########## Step 4b: verify the install actually landed ##########"
# Hyprland silently autogenerates its own default config (and only warns about it
# from inside the session) whenever it finds nothing at ~/.config/hypr/hyprland.conf.
# Catch that here instead of leaving it to be discovered later.
INSTALL_OK=1
if ! cmp -s "$CONF/hypr/hyprland.conf" "$HOME/.config/hypr/hyprland.conf" 2>/dev/null; then
    INSTALL_OK=0
    echo "FAIL: ~/.config/hypr/hyprland.conf does not match this repo's config."
    echo "      Hyprland will keep using its own autogenerated default (only SUPER+Q"
    echo "      and SUPER+M bound) until this is fixed. Run:"
    echo "        cp -r config/hypr config/waybar config/kitty config/rofi config/mako config/wlogout config/gtk-3.0 config/gtk-4.0 ~/.config/"
    echo "        cp config/starship.toml ~/.config/starship.toml"
else
    echo "OK: ~/.config/hypr/hyprland.conf matches the repo — hyprveil's config is live."
fi

echo
echo "########## Step 5: reload ##########"
if [ "$INSTALL_OK" -ne 1 ]; then
    echo "Skipping reload — fix the config copy above first, then reload manually with:"
